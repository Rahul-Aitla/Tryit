  ...
  return (
    <div className="min-h-full pb-32">
      <div className="max-w-[1500px] mx-auto px-4 sm:px-8 pt-16">
        ...
        >
          <div className="mb-8 sm:mb-12">
            ...
            <button
              ...
              className="flex items-center gap-2.5 px-5 py-2.5 border border-hairline rounded-lg text-sm font-semibold transition-all active:scale-95 text-muted-foreground hover:text-ink bg-background"
            >
              {sidebarCollapsed ? (
                <>
          ...
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-display font-normal tracking-[-1.5px] leading-[1] mb-5 mt-2">
        ...
        >
          <div className="mb-10 sm:mb-16 p-1 bg-muted border border-hairline rounded-md max-w-5xl mx-auto flex items-center justify-between text-xs font-semibold text-muted-foreground overflow-x-auto no-scrollbar">
          {[
            ...
            <div key={item.step} className="flex items-center flex-1 justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-2.5 relative transition-all duration-300 whitespace-nowrap">
              {item.active ? (
                <>
                  <div className="absolute inset-0 bg-ink text-background rounded-[3px] -z-10" />
                  <span className="text-background font-mono">{item.step}</span>
                  <span className="text-background font-medium hidden sm:inline">{item.label}</span>
                  <span className="text-background font-medium sm:hidden">Ingest</span>
                </>
              ) : (
                <>
              ...
              {idx < 4 && <div className="h-px w-4 sm:w-8 bg-hairline mx-auto hidden md:inline ml-2 sm:ml-4" />}
            </div>
          ))}
          </div>
        </motion.div>
        ...
        {mobileSidebarOpen && (
          <>
            ...
            <motion.div
              ...
              // @ts-expect-error framer-motion type
              className="fixed top-0 left-0 bottom-0 z-[110] w-[300px] bg-background border-r border-hairline shadow-2xl lg:hidden overflow-y-auto"
            >
              <div className="flex items-center justify-between px-6 py-5 border-b border-hairline">
                <span className="text-sm font-bold tracking-tight">Project History</span>
                <button
                  onClick={() => setMobileSidebarOpen(false)}
